...
import {
  promotionsSelector,
  isPendingPromotionsSelector,
} from "store/promotions/selectors";
...
export default () => {
  ...
  const project = useMemo(() => projects.find(({ id }) => id === projectId), [
    projectId,
    projects,
  ]);
  ...
  return (
    <Form
      ...
      additionalControls={
        <CustomButton href={`/#${ROUTES.PROJECTS}/${projectId}`}>
          Go to Project
        </CustomButton>
      }
    >
        ...
        {!isCreateMode && (
          <div className={styles.images}>
            <ImageUpload
              ...
              imageName="image"
              onComplete={(url) =>
                dispatch(updatePromotion({ image: url, id: selectedPromoId }))
              }
            />
            <ImageUpload
              label="Share image"
              ...
              onComplete={(url) =>
                dispatch(
                  updatePromotion({ shareImage: url, id: selectedPromoId })
                )
              }
            />
          </div>
